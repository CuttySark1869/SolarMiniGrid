# -*- coding: utf-8 -*-
"""
Description: the following script is to creat Pandas dataframe for storing Scom register info
...
# port
port_name = ['COM0', 'COM1', 'COM2', 'COM3', 'COM4', 'COM5', 'COM6', 'COM7', 'COM8',
...
# 1. Import Register Dict
# -------------------------
labels = ['object_id', 'description', 'data_format']
...
verbose_df = pd.DataFrame({'verbose': []})
BSP_setting = pd.concat([BSP_setting, verbose_df], axis=1)
BSP_setting.fillna(verbose, inplace=True)

src_addr_df = pd.DataFrame({'src_addr': []})
BSP_setting = pd.concat([BSP_setting, src_addr_df], axis=1)
BSP_setting.fillna(src_addr, inplace=True)

dst_addr_df = pd.DataFrame({'dst_addr': []})
BSP_setting = pd.concat([BSP_setting, dst_addr_df], axis=1)
BSP_setting.fillna(BSP_addr, inplace=True)

object_type_df = pd.DataFrame({'object_type': []})
BSP_setting = pd.concat([BSP_setting, object_type_df], axis=1)
BSP_setting.fillna(parameter_object_object_type, inplace=True)

property_id_df = pd.DataFrame({'property_id': []})
BSP_setting = pd.concat([BSP_setting, property_id_df], axis=1)
BSP_setting.fillna(parameter_object_property_Id_flash, inplace=True)


cols = ['verbose', 'src_addr', 'dst_addr', 'object_type', 'property_id']
BSP_setting[cols] = BSP_setting[cols].astype(int)
# -----------------------------------------------------
# BSP info parameters
...
verbose_df = pd.DataFrame({'verbose': []})
BSP_info = pd.concat([BSP_info, verbose_df], axis=1)
BSP_info.fillna(verbose, inplace=True)

src_addr_df = pd.DataFrame({'src_addr': []})
BSP_info = pd.concat([BSP_info, src_addr_df], axis=1)
BSP_info.fillna(src_addr, inplace=True)

dst_addr_df = pd.DataFrame({'dst_addr': []})
BSP_info = pd.concat([BSP_info, dst_addr_df], axis=1)
BSP_info.fillna(BSP_addr, inplace=True)

object_type_df = pd.DataFrame({'object_type': []})
BSP_info = pd.concat([BSP_info, object_type_df], axis=1)
BSP_info.fillna(user_info_object_object_type, inplace=True)

property_id_df = pd.DataFrame({'property_id': []})
BSP_info = pd.concat([BSP_info, property_id_df], axis=1)
BSP_info.fillna(user_info_object_property_Id, inplace=True)


cols = ['verbose', 'src_addr', 'dst_addr', 'object_type', 'property_id']
BSP_info[cols] = BSP_info[cols].astype(int)
# -----------------------------------------------
# RCC parameters
...
verbose_df = pd.DataFrame({'verbose': []})
RCC = pd.concat([RCC, verbose_df], axis=1)
RCC.fillna(verbose, inplace=True)

src_addr_df = pd.DataFrame({'src_addr': []})
RCC = pd.concat([RCC, src_addr_df], axis=1)
RCC.fillna(src_addr, inplace=True)

dst_addr_df = pd.DataFrame({'dst_addr': []})
RCC = pd.concat([RCC, dst_addr_df], axis=1)
RCC.fillna(RCC_addr, inplace=True)

object_type_df = pd.DataFrame({'object_type': []})
RCC = pd.concat([RCC, object_type_df], axis=1)
RCC.fillna(parameter_object_object_type, inplace=True)

property_id_df = pd.DataFrame({'property_id': []})
RCC = pd.concat([RCC, property_id_df], axis=1)
RCC.fillna(parameter_object_property_Id_flash, inplace=True)


cols = ['verbose', 'src_addr', 'dst_addr', 'object_type', 'property_id']
RCC[cols] = RCC[cols].astype(int)
# -------------------------------------
...
verbose_df = pd.DataFrame({'verbose': []})
Xtender_setting = pd.concat([Xtender_setting, verbose_df], axis=1)
Xtender_setting.fillna(verbose, inplace=True)

src_addr_df = pd.DataFrame({'src_addr': []})
Xtender_setting = pd.concat([Xtender_setting, src_addr_df], axis=1)
Xtender_setting.fillna(src_addr, inplace=True)

dst_addr_df = pd.DataFrame({'dst_addr': []})
Xtender_setting = pd.concat([Xtender_setting, dst_addr_df], axis=1)
Xtender_setting.fillna(XTM_addr, inplace=True)

object_type_df = pd.DataFrame({'object_type': []})
Xtender_setting = pd.concat([Xtender_setting, object_type_df], axis=1)
Xtender_setting.fillna(parameter_object_object_type, inplace=True)

property_id_df = pd.DataFrame({'property_id': []})
Xtender_setting = pd.concat([Xtender_setting, property_id_df], axis=1)
Xtender_setting.fillna(parameter_object_property_Id_flash, inplace=True)

cols = ['verbose', 'src_addr', 'dst_addr', 'object_type', 'property_id']
Xtender_setting[cols] = Xtender_setting[cols].astype(int)
# -------------------------------------------------------------
...
verbose_df = pd.DataFrame({'verbose': []})
Xtender_info = pd.concat([Xtender_info, verbose_df], axis=1)
Xtender_info.fillna(verbose, inplace=True)

src_addr_df = pd.DataFrame({'src_addr': []})
Xtender_info = pd.concat([Xtender_info, src_addr_df], axis=1)
Xtender_info.fillna(src_addr, inplace=True)

dst_addr_df = pd.DataFrame({'dst_addr': []})
Xtender_info = pd.concat([Xtender_info, dst_addr_df], axis=1)
Xtender_info.fillna(XTM_addr, inplace=True)

object_type_df = pd.DataFrame({'object_type': []})
Xtender_info = pd.concat([Xtender_info, object_type_df], axis=1)
Xtender_info.fillna(user_info_object_object_type, inplace=True)

property_id_df = pd.DataFrame({'property_id': []})
Xtender_info = pd.concat([Xtender_info, property_id_df], axis=1)
Xtender_info.fillna(user_info_object_property_Id, inplace=True)

cols = ['verbose', 'src_addr', 'dst_addr', 'object_type', 'property_id']
Xtender_info[cols] = Xtender_info[cols].astype(int)

# 2. Create DataFrame
# ---------------------------------------
Register_DF = pd.concat([BSP_setting, BSP_info, RCC, Xtender_setting, Xtender_info], axis=0)
Register_DF = Register_DF.reset_index(drop=True)
Register_DF.data_format.replace(['LONG ENUM', 'SHORT ENUM'], ['ENUM', 'ENUM'], inplace=True)
########################################################################################################
