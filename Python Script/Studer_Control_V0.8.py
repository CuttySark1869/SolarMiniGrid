...
def grid_feeding(port_index, action, max_current, start_time, end_time):
  ...
  r1127.write_RAM(port_index, action)
...
def data_collection(port_index):
  current_datetime, battery_SOC, battery_current, \
      battery_voltage, battery_power, AC_out_current, \
      AC_out_voltage, AC_out_power = read_data(port_index)
  global i
  datetime_array[i, 0] = current_datetime
  b_SOC_array[i, 0] = battery_SOC
  b_I[i, 0] = battery_current
  b_V[i, 0] = battery_voltage
  b_P[i, 0] = battery_power
  ac_out_I[i, 0] = AC_out_current
  ac_out_V[i, 0] = AC_out_voltage
  ac_out_P[i, 0] = AC_out_power
  i += 1
  if i == sample_no:
    job_data_collection.remove()
    print 'Data collection finished!'
    datetime_array = pd.DataFrame(datetime_array)
    b_SOC_array = pd.DataFrame(b_SOC_array)
    b_I_array = pd.DataFrame(b_I_array)
    b_V_array = pd.DataFrame(b_V_array)
    b_P_array = pd.DataFrame(b_P_array)
    ac_out_I_array = pd.DataFrame(ac_out_I_array)
    ac_out_V_array = pd.DataFrame(ac_out_V_array)
    ac_out_P_array = pd.DataFrame(ac_out_P_array)
    collected_data = pd.concat([datetime_array, b_SOC_array, b_I_array,
                                b_V_array, b_P_array, ac_out_I_array, ac_out_V_array, ac_out_P_array], axis=1)
    save_dir = cwd + '\\DischargeData\\'
...
def main():
  ...
  ac_out_V_array = np.zeros([sample_no, 1])
  ac_out_I_array = np.zeros([sample_no, 1])
  ac_out_P_array = np.zeros([sample_no, 1])
  global i
  i = 0
  # system init
  system_init(1)
  ...
  job_data_collection = sched.add_job(data_collection, 'cron', second='0,15,30,45')
