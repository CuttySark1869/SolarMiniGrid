...
class ScomCommand:
  ...
  def write_cmd(self, value):
    write_cmd = ('--port={} --verbose={} write_property src_addr={} dst_addr={} object_type={} '
                 ...
                 format(self.port, self.verbose, self.src_addr, self.dst_addr, self.object_type, self.object_id, self.property_id, self.data_format, value))
    return write_cmd

  def write_cmd_RAM(self, value):
    write_cmd_RAM = ('--port={} --verbose={} write_property src_addr={} dst_addr={} object_type={} '
                     'object_id={} property_id={} format={} value={}'.
                     format(self.port, self.verbose, self.src_addr, self.dst_addr, self.object_type, self.object_id, self.parameter_object_property_Id_RAM, self.data_format, value))
    return write_cmd_RAM

  def read_cmd(self):
    read_cmd = ('--port={} --verbose={} read_property src_addr={} dst_addr={} object_type={} '
                'object_id={} property_id={} format={}'.
                format(self.port, self.verbose, self.src_addr, self.dst_addr, self.object_type, self.object_id, self.property_id, self.data_format))
    return read_cmd
  ...
  def write_RAM(self, value):
    write_cmd = self.write_cmd_RAM(value)
    scom_output = subprocess.Popen(dir_scom + write_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    str_output = scom_output.stdout.readlines()
    if ScomCommand.display_output:
      for line in str_output:
...
def synchronise_time():
  ...
  time_delta_second = r5002.write_RAM(target_time_delta_second)
  system_datetime = get_system_time()
  return system_datetime
# -------------------------
if rcc_init:
...
if bsp_init:
  if battery_setting == 1:
    ...
    r6059.write_RAM(10)
    elapsed_time = time.time() - start_time
    print 'BSP(DK) initializaiton finished, took ' + str(elapsed_time) + ' seconds'
  # --------------------------------------------------------------------------------------
    ...
    # -----------------------
    elapsed_time = time.time() - start_time
    print 'BSP (Lab) initializaiton finished, took ' + str(elapsed_time) + ' seconds'
  # --------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------
