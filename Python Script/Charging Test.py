...
def data_collection(port_index):
  ...
  global b_V_array
  global b_P_array
  global ac_in_I_array
  global ac_in_V_array
  global ac_in_P_array
  print str(current_datetime)

  datetime_array[i, 0] = current_datetime
  b_SOC_array[i, 0] = battery_SOC
  b_I_array[i, 0] = battery_current
  b_V_array[i, 0] = battery_voltage
  b_P_array[i, 0] = battery_power
  ac_in_I_array[i, 0] = AC_in_current
  ac_in_V_array[i, 0] = AC_in_voltage
  ac_in_P_array[i, 0] = AC_in_power
  i += 1

  if i % 4 == 0:
    df_datetime_array = pd.DataFrame(datetime_array[:i, 0])
    df_b_SOC_array = pd.DataFrame(b_SOC_array[:i, 0])
    df_b_I_array = pd.DataFrame(b_I_array[:i, 0])
    df_b_V_array = pd.DataFrame(b_V_array[:i, 0])
    df_b_P_array = pd.DataFrame(b_P_array[:i, 0])
    df_ac_in_I_array = pd.DataFrame(ac_in_I_array[:i, 0])
    df_ac_in_V_array = pd.DataFrame(ac_in_V_array[:i, 0])
    df_ac_in_P_array = pd.DataFrame(ac_in_P_array[:i, 0])
    collected_data = pd.concat([df_datetime_array, df_b_SOC_array, df_b_I_array,
                                df_b_V_array, df_b_P_array, df_ac_in_I_array, df_ac_in_V_array, df_ac_in_P_array], axis=1)
    collected_data.columns = ['DateTime', 'Battery_SOC', 'Battery_Current', 'Battery_Voltage',
                              'Battery_Power', 'AC_In_Current', 'AC_In_Voltage', 'AC_In_Power']
    save_dir = cwd + '\\ChargeData\\'
    if not os.path.exists(save_dir):
      os.makedirs(save_dir)
    file_name = 'Charge_data.csv'
    collected_data.to_csv(save_dir + file_name, index=True)
  if i == sample_no:
...
def main():
  ...
  sample_no = 4 * 60 * 24
#     sample_no = 4
  datetime_array = np.zeros([sample_no, 1], dtype='datetime64[ns]')
  b_SOC_array = np.zeros([sample_no, 1])
  b_I_array = np.zeros([sample_no, 1])
  b_V_array = np.zeros([sample_no, 1])
  b_P_array = np.zeros([sample_no, 1])
  ac_in_V_array = np.zeros([sample_no, 1])
  ac_in_I_array = np.zeros([sample_no, 1])
  ac_in_P_array = np.zeros([sample_no, 1])
  # system init
  system_init(1)
